...
var EasyNodeMaster = /** @class */ (function (_super) {
    ...
    function EasyNodeMaster(config) {
        ...
        _this.clusterOptions = Object.assign({
            logs: {
                normal: './runtime.log',
                error: './error.log'
            }
        }, config);
        _this.normal = fs_1.openSync(path_1.join(process.cwd(), config.logs.normal), 'a');
        _this.error = fs_1.openSync(path_1.join(process.cwd(), config.logs.error), 'a');
        return _this;
    }
